...
const buildAssignedEmailHtml = ({
    ...
    const safeTransferredBy = escapeHtml(transferred_by_name || 'Commissioner');
    ...
    return `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;line-height:1.5;">
        <h2 style="margin:0 0 12px;color:#222;">New Case Assigned</h2>
        <p style="margin:0 0 12px;color:#444;">Hello ${safeInspector},</p>
        <p style="margin:0 0 12px;color:#444;">
          A new case (<strong>${safeTitle}</strong>) has been transferred to you by ${safeTransferredBy}.
        </p>
        ...
        <p style="margin:0 0 12px;color:#444;">Please review the case details and take necessary action.</p>
...
async function sendCaseAssignedEmail({ email, ...payload }) {
    ...
    if (!targetEmail) {
        throw new Error('Recipient email is required to send case assigned email');
    }
    ...
    await transporter.sendMail({
        ...
        to: targetEmail,
        subject: 'New Case Transferred to You',
        html: buildAssignedEmailHtml(payload),
    });

    return { status: 'sent' };
}

module.exports = { sendCaseAssignedEmail };
